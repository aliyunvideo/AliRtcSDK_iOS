...
/**
 外部音频源配置属性
 */
@interface AliRtcExtAudioSourceConfig : NSObject

@property (nonatomic, assign) BOOL enable;         // 是否启用外部音频
@property (nonatomic, assign) BOOL publish;        // 是否推流到远端
@property (nonatomic, assign) BOOL playout;        // 是否本地渲染播放
@property (nonatomic, assign) BOOL mixedWithMic;   // 是否与Mic混音
 ...
 *  外部音频裸数据格式
 */
@interface AliRtcExtAudioFrame : NSObject

@property (nonatomic, assign) long dataPtr;          // 音频帧数据,不建议超过40ms
@property (nonatomic, assign) int numOfSamples;      // 音频数据长度
...
@property (nonatomic, assign) double displayAspectRatio;
@property (nonatomic, assign) long long timeStamp;
...
/**
* @brief 设置SDK日志文件保存大小（单文件）
* @param sizeInKBytes 日志文件保存上限，单位KB
* @return 0为成功，非0失败
* @note 限制日志单个文件大小。整体日志，最多不超过5个文件（5*sizeInKBytes）大小体积内。
*       当设置单个日志文件小于1024KB时，单个日志文件仍为1024KB
*/
+ (int)setLogFileSize:(unsigned int)sizeInKBytes;
...
/**
 * 禁用或重新启用本地视频采集
 * @param enable  NO表示停止视频采集；YES表示恢复正常
 */
...
/**
 * @brief 设置外部音频输入源
 * @param audioSourceConfig 外部音频源配置
 * @return return>=0 Success, return<0 Failure
 */
- (int)setExternalAudioSource:(AliRtcExtAudioSourceConfig *)audioSourceConfig;
 ...
 * @note 如果返回值为errorCode中的AliRtcErrAudioBufferFull，代表当前buffer队列塞满，需要等待后再继续输送数据，，建议等待20ms
 */
- (int)pushExternalAudioFrameRawData:(AliRtcExtAudioFrame *)audioFrame;
 ...
 * @param vol 音量 0-100
 */
- (int)setExternalAudioPlayoutVolume:(int)vol;

/**
 * @brief 获取本地播放音量
 * @return vol 音量
 */
- (int)getExternalAudioPlayoutVolume;
